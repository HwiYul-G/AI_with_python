...
aids = input("손흥민 선수는 몇 도움을 했나요? ")
score_me = input("손흥민 선수의 팀이 넣은 골의 수는? ")
score_you = input("상대 팀이 넣은 골의 수는? ")
...
if score_me > score_you:
    ...
    news = news + "손흥민 선수의 팀이 " + score_me+"골을 넣어 "+score_you+"골을 넣은 상대 팀에게 패배했습니다."
else:
    news = news+"두 팀은 " + score_me + "대" + score_you + "로 비겼습니다."

if int(goals) > 0 and int(aids) > 0:
    news = news + "손흥민 선수는 " + goals+"골에 도움 "+aids+"개로 승리를 크게 이글었습니다."
elif int(goals) > 0 and int(aids) == 0:
    news = news + "손흥민 선수는 " + goals+"골로 승리를 이끌었습니다."
else:
    news = news + "아쉽게도 이번 경기에서 손흥민의 발끝은 침북을 지켰습니다."
